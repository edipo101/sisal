	...
	<div class="col-md-6">
  	...
  	<div class="col-md-6">
		<div class="form-group">
			...
			{{ Form::select('roles[]',$roles,isset($user) ? $user->roles->pluck('id') : null,['class'=>'form-control select', 'multiple' => 'multiple']) }}
		</div>
  	</div>
</div>
<div class="row">
	<div class="col-md-6">
        <div class="form-group{{ $errors->has('almacen_id') ? ' has-error' : '' }}">
            ...
            {{ Form::select('almacen_id', $almacenes, null,['class'=> 'form-control','id' => 'almacen_id']) }}

            @if ($errors->has('almacen_id'))
                <span class="help-block">
                    <strong>{{ $errors->first('almacen_id') }}</strong>
                </span>
            @endif
        </div>
    </div>
